...
exports.updateUserRole = async (req, res, next) => {
  try {
    ...
    // Prevent changing role of bypass users
    if (targetUserId === 'root_admin_bypass') {
      return res.status(400).json({ success: false, message: 'Cannot modify bypass user role' });
    }
...
exports.toggleUserActive = async (req, res, next) => {
  try {
    ...
    if (targetUserId === 'root_admin_bypass') {
      return res.status(400).json({ success: false, message: 'Cannot toggle bypass user status' });
    }
...
exports.resetUserPassword = async (req, res, next) => {
  try {
    ...
    if (targetUserId === 'root_admin_bypass') {
      return res.status(400).json({ success: false, message: 'Cannot reset bypass user password' });
    }
